...
static func build_solid_branch(
	...
	var dense_samples: Array = densify_samples(samples, branch_length)
...
static func get_profile_tip_offset(samples: Array) -> Vector3:
	if samples.is_empty():
		return Vector3.ZERO
	var centers := compute_sample_centers(samples)
	return centers[-1]
...
static func interpolate_sample_at_dist(samples: Array, dist: float) -> Dictionary:
	...
	for i in range(samples.size() - 1):
		...
		if dist <= b_dist or i == samples.size() - 2:
			...
			return {
				"dist": dist,
				"dir": segment_dir.slerp(b.get("dir", Vector3.UP), t).normalized(),
