...
static func apply_at_tip(graph, tip_id: int, pattern) -> void:
	...
	var rng: RandomNumberGenerator = graph.get_rng()
	...
	while index < production.length():
		...
		match symbol:
			...
			"^":
				var pitch_up: float = -_jittered_angle(angle, pattern, rng)
				heading = heading.rotated(left, pitch_up).normalized()
				turtle_up = turtle_up.rotated(left, pitch_up).normalized()
				left = turtle_up.cross(heading).normalized()
			"\\":
				var roll: float = _jittered_angle(angle, pattern, rng)
			...
			"/":
				var roll_neg: float = -_jittered_angle(angle, pattern, rng)
			...
			_:
				if _is_module_symbol(symbol):
					var spawn_dir: Vector3 = _spread_direction(heading, pattern, rng)
					spawns.append({
						"direction": spawn_dir,
						"lsymbol": symbol,
						"turtle_up": SpatialGrowth.random_turtle_up(
							spawn_dir,
							rng,
							pattern.lateral_roll_spread_deg
						),
	...
	for spawn in spawns:
		...
		if not GrowthLimits.can_add_node(graph.nodes.size()):
			break
		if pattern.lateral_skip_chance > 0.0 and rng.randf() < pattern.lateral_skip_chance:
			continue
	...
	if parent_continues:
		tip.direction = _spread_direction(heading, pattern, rng)
		tip.turtle_up = SpatialGrowth.random_turtle_up(
			tip.direction,
			rng,
			pattern.lateral_roll_spread_deg * 0.5
		)
...
static func _jittered_angle(base_angle: float, pattern, rng: RandomNumberGenerator) -> float:
...
static func _spread_direction(direction: Vector3, pattern, rng: RandomNumberGenerator) -> Vector3:
